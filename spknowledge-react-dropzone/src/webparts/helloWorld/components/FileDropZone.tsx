...
const FileDropZone: FC<IFileDropZoneProps> = (props) => {
      ...
      fileReader.onload = ((fle) => {
        return (e) => {
          tempDocsToUpload.push({
            ...
            size:
              fle.size > 1048576
                ? Math.round(fle.size / 1024 / 1024).toString() + "MB"
                : Math.round(fle.size / 1024).toString() + "KB",
            actualSize: fle.size,
  ...
  const _handleFileChange = async (files) => {
    ...
    if (tempDocsToUpload.length > 0) {
      var diffFiles: any[] = differenceBy(files, tempDocsToUpload, (f) =>
        trim(f.name)
      );
    ...
    if (batch.length > 0) {
      ...
      if (tempDocsToUpload.length <= fle_maxfilesallowed) {
        if (props.useAsyncUpload) {
          for (let fletoupload of tempDocsToUpload) {
            if (!fletoupload.uploadStatus) {
              let uploadStatus: boolean = await props.tempUpload(
                fletoupload.trimmedName,
                fletoupload.content
              );
        ...
        props.dropCallback(tempDocsToUpload);
        setErrorMessage(
          "Sorry, you have reached the max number and no more files can be uploaded!"
        );
      }
    }
  ...
  const _fileValidation = (file: File): FileError | FileError[] => {
    if (file.size > fle_maxFileLength) {
      return {
        code: "file_max_size",
        message: `File is greater than max size of ${
          fle_maxFileLength / 1024 / 1024
        }MB`,
      };
    }
    return null;
   ...
   */
  const _customFileProperties = async (
    e
  ...
  const fileRejectionItems = fileRejections.map(({ file, errors }) => (
    <li key={file.name}>
      ...
      <ul>
        {errors.map((e) => (
          <li key={e.code} style={{ fontWeight: "normal" }}>
            {e.message}
          </li>
        ))}
      </ul>
    ...
    <section className={styles.dropZoneContainer}>
      <div
        {...getRootProps({
          className: css(
            styles.dropzone,
            disableUpload ? styles.dropZonedisabled : ""
          ),
        })}
      >
        <input {...getInputProps()} />
      ...
      <div className={styles.dropZoneInfo}>
        {`Maximum of ${fle_maxfilesallowed} files can be uploaded. Each file can have a max size of ${
          fle_maxFileLength / 1024 / 1024
        }MB. The total file size allowed per case is ${
          fle_maxFileListLength / 1024 / 1024
        }MB.`}
      </div>
