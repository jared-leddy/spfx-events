...
const SampleFileUpload: FC<ISampleFileUploadProps> = (props) => {
  ...
  const _uploadFiles = async (
    ...
    try {
      let tmpFileUploaded = await uploadFiles(
        `${props.webSerUrl}/Shared Documents/`,
        filename,
        filecontent
      );
      let tempFiles: any[] = tempFilesUploaded;
      tempFiles.push({
        uploadedFileName: tmpFileUploaded.data.Name,
        trimmedName: filename,
        FileServerRelUrl: tmpFileUploaded.data.ServerRelativeUrl,
      });
      ...
      tempFilesUploaded.map((tmpfile) => {
        var fil = find(
          tempDocsToUpload,
          (f: IFileUploadInfo) => f.trimmedName === tmpfile.trimmedName
        );
        if (fil) {
  ...
  return (
    <div className={styles.sampleFileUpload}>
      {uploadingFiles && (
        <ContentLoader
          loaderType={LoaderType.Indicator}
          loaderMsg={"Processing files..."}
        />
      )}
      <FileDropZone
        dropCallback={_afterSelectingFiles}
        documentToUpload={docsToUpload}
        tempUpload={_uploadFiles}
        clearMessage={clearFileErrorMsg}
        showUploadProgress={_showUploadProgress}
        hideUploadProgress={_hideUploadProgress}
        disableUpload={uploadingFiles}
      ...
      {docsToUpload &&
        ...
        docsToUpload.map((doc) => (
          <ul>
            <div>
              {doc.name}{" "}
              {doc.FileServerRelUrl ? `('${doc.FileServerRelUrl}')` : ""}
            </div>
          </ul>
        ))}
      {!props.asyncUpload && (
        <div style={{ float: "right" }}>
          <PrimaryButton
            ...
            onClick={_uploadOnSave}
            disabled={uploadingFiles || docsToUpload.length <= 0}
            style={{ float: "right" }}
          ></PrimaryButton>
        </div>
      )}
